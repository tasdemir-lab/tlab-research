...
description: |
  ...
  Launches 5 parallel specialist agents to find stale references, broken paths,
...
- **Skills** live in the plugin (not `.claude/skills/`). `AGENTS.md` (canonical) lists them as `/tlab:*`.
...
Launch these 5 agents simultaneously using `Agent` with `subagent_type=general-purpose`:

#### Agent 1: AGENTS.md & README Accuracy
Focus: `AGENTS.md` (canonical), `CLAUDE.md` (pointer), `README.md`
- Skills table in `AGENTS.md` lists `/tlab:*` commands that match available plugin skills
- `CLAUDE.md` is a pointer that `@import`s `AGENTS.md` and contains the literal `tlab` marker; **no
  `GEMINI.md` exists** (Antigravity loads `AGENTS.md` directly)
- Folder structure described in README matches actual directories on disk
- Commands listed in `AGENTS.md` and README are accurate and runnable
...
Focus: `README.md`, `AGENTS.md`, `code/`, `paper/`, `.gitignore`
...
- Bibliography file referenced in `AGENTS.md` exists at the stated location
...
Focus: All skill files (plugin `skills/*/SKILL.md` AND user-level `~/.claude/skills/*/SKILL.md`), global `~/.claude/CLAUDE.md`, project `AGENTS.md` (canonical instruction file), `.claude/rules/*.md`
- Extract explicit preferences from the instruction files (project `AGENTS.md` + global `~/.claude/CLAUDE.md`): git merge strategy, commit style, build commands, code style, "always/never" instructions
