...
- **Follow R code conventions** (the `r-code-conventions` rule -- loaded automatically from user level)
...
1. Follow the `r-code-conventions` rule for coding standards
2. Create R script with proper header (title, author, purpose, inputs, outputs)
3. Load required packages at top (`library()`, never `require()`). Always include `library(rio)`
   — it is the project default for data import/export (per AGENTS.md § R Data Conventions).
4. Set seed once at top: `set.seed(42)`
5. **Inspect the raw file before importing** — many local datasets use European/Turkish
   conventions (`;` delimiter, `,` decimal). Check encoding and delimiter:
   ```r
   readLines(path, n = 3)   # eyeball the first 3 lines
   ```
6. Load with `rio::import()`, passing explicit `sep =`, `dec =`, and `encoding =` when the
   inspection reveals non-default conventions. Run `janitor::clean_names()` after import if
   column names contain whitespace or non-ASCII characters.
...
3. **Snapshot dependencies if any new packages were installed during this analysis:**
   ```r
   renv::status()     # see what changed
   renv::snapshot()   # update renv.lock — required by AGENTS.md § Git Rules
   ```
   Then stage `renv.lock` for the next commit.
4. Run the r-reviewer agent on the generated script:
