...
4. **If format includes `html`, verify the output is self-contained.** The project convention
   (global CLAUDE.md § Quarto) is `embed-resources: true`, so HTML can be shared without
   broken image links. Check the source YAML, and if absent, warn the user:
   ```bash
   grep -q 'embed-resources: true' code/FILENAME.qmd \
     || echo "WARN: HTML output is not embed-resources — assets may break when shared."
   ```
